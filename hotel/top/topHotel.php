...
GROUP BY
    ...
    c.OutboundOriginTlc, c.OutboundArrivalTlc
        ...
        $codes = ($code === 'all') ? "'SS:ERR:11191','SS:ERR:1137','SS:ERR:11200'" : "'" . addslashes($code) . "'";
...
GROUP BY
    PlayerProvider, b.Code, c.PlayerBrand ,c.HotelGiataCode, c.HotelAccomodation,
    c.AdultCounts, c.ChildrenCounts, c.InfantCounts, a.[User],
    CONCAT(FORMAT(c.OutboundDate,'dd.MM.yy'),' - ',FORMAT(c.InboundDate,'dd.MM.yy')),
    c.OutboundOriginTlc, c.OutboundArrivalTlc
        ";
        ...
        echo "<div class='table-responsive'>
              ...
              <tr>
                <th>Provider</th><th>Brand</th><th>GiataCode</th><th>Accomodation</th>
                <th>Adults</th><th>Children</th><th>Infants</th>
    ...
    <div class="row">
      <div class="col-md-6 mb-3">
        ...
        <select name="code" class="form-select" id="code">
          ...
          <option value="SS:ERR:11200" <?= (($_POST['code'] ?? '') === 'SS:ERR:11200') ? 'selected' : '' ?>>SS:ERR:11200</option>
        </select>
      </div>
  </div>
      <div class="row">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['providerAjax'])) {
    ...
    $codes = ($code === 'all') ? "'SS:ERR:11191','SS:INFO:00443','SS:ERR:11200'" : "'" . addslashes($code) . "'";
...
function updateCodeMessage() {
    ...
    if (code === 'SS:ERR:11191') {
        html = '<div class="alert alert-danger">All rooms are booked out in the hotel</div>';
    } 
    else if (code === 'SS:INFO:00443') {
        html = '<div class="alert alert-warning">The connection to the service provider is disturbed</div>';
    } 
    else if (code === 'SS:ERR:11200') {
        html = '<div class="alert alert-warning">hotel is not activated for usage</div>';
    } 
    else if (code === 'SS:ERR:1137') {
        html = '<div class="alert alert-secondary">General error</div>';
    } 
    else {
        html = '';
    }
