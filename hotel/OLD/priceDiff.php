...
MainDetail AS (
    ...
    WHERE ph.playerHotelPrice IS NOT NULL
      ...
      AND COALESCE(he.HotelEKPriceInSalesCY,0) <> COALESCE(ph.playerHotelPrice,0)
      AND (COALESCE(he.HotelEKPriceInSalesCY,0) - COALESCE(ph.playerHotelPrice,0)) > 400
)
SELECT
    PlayerBrand,
    Giata AS HotelGiataCode,
    HotelCode,
    Rise,
    von AS Outbound_flight_route,
    nach AS Inbound_flight_route,
    PlyHPrice AS playerHotelPrice,
    HotelEK AS HotelEKPriceInSalesCY,
    PriceDiff,
    COUNT(*) OVER (
        PARTITION BY PlayerBrand, Giata, HotelCode, Rise, von, nach, PlyHPrice, HotelEK, PriceDiff
    ) AS BAs
FROM MainDetail
GROUP BY
    PlayerBrand, Giata, HotelCode, Rise, von, nach, PlyHPrice, HotelEK, PriceDiff
...
document.addEventListener("DOMContentLoaded", function () {
    const getCellValue = (row, index) => {
        ...
        return isNaN(cell) ? cell.trim() : parseFloat(cell.replace(/,/g, ''));
    };
