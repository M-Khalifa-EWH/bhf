    ...
    if ($table === 'table1') {
        $query = "
            SELECT PlayerProvider, b.Code AS Code,
                   ...
                   c.OutboundOriginTlc AS Origin, c.OutboundArrivalTlc AS destination,
                   COUNT(a.id) AS ct
...
$(document).ready(function() {
    ...
    var tableName = '<?php echo addslashes($table); ?>';
    var codeParam = <?php echo json_encode($code ?? ''); ?>;


    var shouldHide = (codeParam === 'SS:ERR:18');

    var columnDefs = [];

  

    columnDefs.push({ targets: 1, visible: !shouldHide });


    if (tableName === 'table1') {
        columnDefs.push({ targets: 2, visible: !shouldHide });
    }

  

    columnDefs.push({ targets: 3, visible: !shouldHide });
   ...
   $('#detailsTable').DataTable({
    ...
    order: [[<?php echo ($table==='table1')?'11':'10'; ?>, 'desc']],
    columnDefs: [
