...
<body>
    ...
    $sql = "SELECT buchungsDatum, Agenturnummer, Buchungsuhrzeit as Uhrzeit, Statusderbuchung, PlayerBrand, Transfertyp, Betrag,Zahlungsart, Hinflugdatum, TrademarkCode, FlugnummerHin,
    AbflugzeitHin, AnkunftszeitHin, AbflugzeitRück, AnkunftszeitRück, AirlineHin, Buchungsnummer, Von, Nach, Nachname, ErwachseneAktiv, ErwachseneInaktiv, KinderAktiv,
    KinderInaktiv, BabiesAktiv, HotelCode, Hotelname, GiataCode, Zimmerart, ZimmerartCode, Verpflegung, AnkunftsdatumHin, AnkunftsdatumRück, Transfer 
    FROM dbo.csv_export_copy 
    WHERE CAST(CONVERT(datetime, CONVERT(varchar, Buchungsdatum, 104), 104) AS date) > DATEADD(MONTH, -1, GETDATE())
    AND Statusderbuchung IN ($statusPlaceholders)            
    ...
    sqlsrv_close($connect);
        ...
        foreach ($data as $row) {
            ...
            <td nowrap>".$row['Statusderbuchung']."</td>
